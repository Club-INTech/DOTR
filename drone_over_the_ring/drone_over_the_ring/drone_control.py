...
class Drone():
    ...
    def __init__(self,
                 ...
                 order_provider: OrderProvider = BasicOrderProvider(),
                 video_provider: VideoProvider = EmptyVideoProvider()) -> None:
        ...
        if (isinstance(self.__order_provider, ROSOrderProvider) and 
            not isinstance(self.__video_rpovider, ROSVideoProvider)) or \
                (isinstance(self.__video_provider, ROSVideoProvider) and 
                 not isinstance(self.__order_rpovider, ROSOrderProvider)):
                    print("You must have both ros providers")
                    sys.exit(1)
    ...
    def run(self) -> None:
        ...
        if self.__use_navigation:
            ...
            while not self.__stop:
                ...
                if self.__drone_state.gate_count >= GATE_NUMBER:
                    self.__stop = True
                    break

                if _desc.type_ == GateType.NO_GATE:
                    self.__drone_state.not_detected_count += 1
                    if self.__drone_state.not_detected_count >= self.__drone_state.not_detected_limit:
                    ...
                    if self.__drone_state.gate_navigation_step == NavigationStep.NOT_DETECTED:
                        self.execute_order("rc 0 0 0 0")
                        self.__drone_state.reset()
                        self.__drone_state.gate_navigation_step = NavigationStep.DETECTED
                ...
                if self.__drone_state.gate_navigation_step == NavigationStep.NOT_DETECTED:
                ...
                elif self.__drone_state.gate_navigation_step == NavigationStep.DETECTED:
                    ...
                    if self.__drone_state.is_at_safe_point():                
                        self.execute_order("rc 0 0 0 0")
                        time.sleep(WAITING)
                        self.__order_provider.forward(int(1.2 * SAFE_DISTANCE*100))
                        self.__drone_state.reset()
                        time.sleep(FORWARD_WAITING)
                    ...
                    speed_x, speed_y, speed_z, speed_yaw = self.__navigator.update_speed(self.__drone_state)
